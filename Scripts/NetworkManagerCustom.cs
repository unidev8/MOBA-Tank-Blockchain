...
namespace TanksMP
{
  ...
  public class NetworkManagerCustom : MonoBehaviourPunCallbacks
  {
    ...
    public override void OnConnectedToMaster()
    {
      ...
      Hashtable expectedCustomRoomProperties = new Hashtable() { { "mode", (byte)PlayerPrefs.GetInt(PrefsKeys.gameMode) }, { "map", PlayerPrefs.GetString(PrefsKeys.map) } };
    ...
    public override void OnJoinRandomFailed(short returnCode, string message)
    {
      ...
      roomOptions.CustomRoomProperties = new Hashtable() { { "mode", (byte)PlayerPrefs.GetInt(PrefsKeys.gameMode) }, { "map", PlayerPrefs.GetString(PrefsKeys.map) } };
    ...
    IEnumerator WaitForSceneChange()
    {
      ...
      {
        while (SceneManager.GetActiveScene().name != "AI_TDM_" + PlayerPrefs.GetString(PrefsKeys.map))
        {
          yield return null;
        }
    ...
    void AddPlayer()
    {
      ...
      if (startPos != null)
      {
        PhotonNetwork.Instantiate(playerPrefabs[prefabId].name, startPos.position, startPos.rotation, 0, data);
      }
      else
      {
        PhotonNetwork.Instantiate(playerPrefabs[prefabId].name, Vector3.zero, Quaternion.identity, 0, data);
      }
    }
