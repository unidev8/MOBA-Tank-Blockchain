...
namespace TanksMP
{
    ...
    public class UIGame : MonoBehaviourPunCallbacks
    {
        ...
        public GameObject royalRoomStateUI;
        public GameObject IsMasterMarkUI;
        public Text masterTime;
        public Text royalLeftTime;
        /// <summary>
        ...
        void Start()
        {
            if (IsMasterMarkUI)
            {
                IsMasterMarkUI.SetActive(PhotonNetwork.IsMasterClient);
                masterPanel.SetActive(PhotonNetwork.IsMasterClient);
            }
        ...
        public override void OnRoomPropertiesUpdate(ExitGames.Client.Photon.Hashtable propertiesThatChanged)
        {
            ...
            if (IsMasterMarkUI)
            {
                SetRoyalTime(PhotonNetwork.CurrentRoom.GetRoyalTime());
                IsMasterMarkUI.SetActive(PhotonNetwork.IsMasterClient);
            }
        }
        ...
        public void SetRoyalTime(int time)
        {
            killCountUI.SetActive(time > 0);
            royalRoomStateUI.SetActive(time <= 0);
            royalLeftTime.text = (Mathf.Abs(time)).ToString();
            if (PhotonNetwork.IsMasterClient)
            {
                masterTime.text = (Mathf.Abs(time)).ToString();
            }
        }
