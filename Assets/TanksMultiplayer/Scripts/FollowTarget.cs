...
namespace TanksMP
{
    ...
    public class FollowTarget : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (cameraMode == CameraMode.Spectator)
            {
                if (Input.GetKeyDown("space"))
                {
                    if (target == null)
                    {
                        target = GameManager.GetInstance().GetRandomPlayerObject()?.transform;
                    }
                    else
                    {
                        if (target.gameObject.TryGetComponent<Photon.Pun.PhotonView>(out Photon.Pun.PhotonView photonView))
                        {
                            target = GameManager.GetInstance().GetRandomPlayerObject(photonView.Owner)?.transform;
                        }
                        else
                        {
                            target = GameManager.GetInstance().GetRandomPlayerObject().transform;
        ...
        void Start()
        {
            ...
            {
                SetCameraMode(CameraMode.Spectator);
            }
        }
        ...
        void LateUpdate()
        {
            if (target == null && DedicatedServerManager.instance.IsServer())
            {
                target = GameManager.GetInstance().GetRandomPlayerObject()?.transform;
                if (target == null)
                {
